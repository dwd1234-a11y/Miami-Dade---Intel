...
class RealForecloseScraper:
    ...
    def _parse_auction_table(self, soup: BeautifulSoup, source: str) -> list[dict]:
        ...
        # Log headers at INFO level so they appear in GitHub Actions logs
        log.info(f"[DEBUG] Table headers ({len(headers)} cols): {headers}")

        col_map = {}
        for i, h in enumerate(headers):
            if "case" in h:
                col_map.setdefault("case_num", i)
            if ("judgment" in h and "amount" in h) or h == "amount":
            ...
            if "assess" in h or ("value" in h and "address" not in h):
                col_map.setdefault("assessed_value", i)

        log.info(f"[DEBUG] col_map: {col_map}")

        # Log first data row to verify mapping
        if len(rows) > 1:
            first_cells = rows[1].find_all(["td", "th"])
            first_row_vals = [c.get_text(strip=True)[:40] for c in first_cells]
            log.info(f"[DEBUG] First data row: {first_row_vals}")
        ...
        for row in rows[1:]:
            try:
                ...
                # Fallback positional map if headers didn't parse
                # Typical RealForeclose order:
